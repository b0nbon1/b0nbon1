...
You can read files from your nodejs application using `fs.readFile` anf `fs.readFileSync`(for synchronous form) methods.
...
You can readfiles from your nodejs application using `fs.writeFile` anf `fs.writeFileSync`(for synchronous form) methods.
...
More details about the parameters are available in [fs.writeFileSync](https://nodejs.org/docs/latest-v15.x/api/fs.html#fs_fs_writefilesync_file_data_options)
